...
void getshrinking(float** rPixelfloat, int imageHeight, int imageWidth,int** M, float** routputArray,int* countArray)
{
    ...
    int bond=0;
    string str;
    for(iterationIndex=1;iterationIndex<32;iterationIndex++)
    {
        count=0;
        ...
        //To count number of rice grains shrinking in every iteration
        for(rowIndex=0;rowIndex<imageHeight;rowIndex++)
        {
            for (colIndex=0;colIndex<imageWidth;colIndex++)
            {
                ...
                for(p=rowIndex-1;p<=rowIndex+1;p++)
                {
                    for(q=colIndex-1;q<=colIndex+1;q++)
                    {
                        ...
                        maskarray1[traversalIndex]=routputArray[row][col];
                        traversalIndex++;
                    }
                }
                if(maskarray1[0]==0 && maskarray1[1]==0 && maskarray1[2]==0 && maskarray1[3]==0 && maskarray1[4]==1 && maskarray1[5]==0 && maskarray1[6]==0 && maskarray1[7]==0 && maskarray1[8]==0)
                {
                    count++;
                }
            }
        }
        countArray[iterationIndex]=count-previousCount;
        
        if(count-previousCount!=0)
        {
            //cout<<"For iteration: "<<iterationIndex<<endl;;
            //cout<<"Count: "<<countArray[iterationIndex]<<endl<<endl;
        }
        previousCount=count;
    }
}

int getrice(unsigned char** rpixelshrinked,int imageHeight, int imageWidth)
{
    ...
    for(rowIndex=0;rowIndex<imageHeight;rowIndex++)
    {
        for(colIndex=0;colIndex<imageWidth;colIndex++)
        {
            if(rpixelshrinked[rowIndex][colIndex]==255) // To count number of rice shrinking to a single white dot in a black background
            {
                count++;
            }
        }
...

{
    ...
    getshrinking(rpixelfloat,imageHeight,imageWidth, M, routputArray,countArray);
    ...
    //To count total number of rice
    int count=getrice(rpixel2Dshrinked,imageHeight,imageWidth);
    cout<<"Total number of rice grains are: "<<endl<<count<<endl;
 ...
 /*
    //Wriring the contents of an array to a file to display the histogram by reading the file in Matlab
    FILE *histFile;

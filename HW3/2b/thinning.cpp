...
void getthinning(float** rPixelfloat, int imageHeight, int imageWidth,int** M, float** routputArray)
{
    ...
    for(iterationIndex=0;iterationIndex<120;iterationIndex++)
    {
        for(rowIndex=0;rowIndex<imageHeight;rowIndex++)
        {
            for(colIndex=0;colIndex<imageWidth;colIndex++)
...

{
    ...
    unsigned char** rpixel2Dthinned=rpixel2Ddummy45;
    ...
    //Performing thinning algorithm
    getthinning(rpixelinvertedfloat,imageHeight,imageWidth, M, routputArray);
    ...
    getoriginalrangevalue(rpixelinvertedfloatfinal,rpixel2Dthinned,imageHeight,imageWidth);
    
    //Combining the R channel to a single image
    combinechannel(rpixel2Dthinned,combinedArray,imageHeight,imageWidth);

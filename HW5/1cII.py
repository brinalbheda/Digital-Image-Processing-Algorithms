...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 17:07:50 2019
...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  27 18:45:07 2019
...
(X_train_input, y_train), (X_test_input, y_test) = mnist.load_data()
...
#X_train_input = mnist_data.train.images 
#print(X_train_input.dtype)
X_train_temp = 255.0 - X_train_input
#X_train = X_train + X_train_temp
#X_train = np.asarray(X_train, dtype=np.float32)
X_train = np.concatenate((X_train_input, X_train_temp),axis=0)
#print(X_train.shape)

X_train = np.reshape(X_train,(120000,28,28,1))
...
#X_valid_input = mnist_data.validation.images
#X_valid_temp = 255.0 - X_valid_input
#X_valid = X_valid + X_valid_temp
#X_valid = np.asarray(X_valid, dtype=np.float32)
#X_valid = np.concatenate((X_valid_input, X_valid_temp))
#y_valid = np.asarray(mnist_data.validation.labels, dtype=np.int32)

#load the testing data, returns in numpy array form
#X_test_input = mnist_data.test.images 
X_test_temp = 255.0 - X_test_input
#X_test = X_test + X_test_temp
#X_test = np.asarray(X_test, dtype=np.float32)
X_test = np.concatenate((X_test_input, X_test_temp),axis=0)
...
def lenet5_architecture_model():
    ...
    model.add(Conv2D(16, kernel_size=(5, 5), strides=1, padding ='same', input_shape=(28,28,1), activation='relu', kernel_initializer='random_uniform'))
